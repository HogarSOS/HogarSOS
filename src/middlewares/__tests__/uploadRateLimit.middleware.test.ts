...
import { uploadRateLimit } from '../authRateLimit.middleware';
...
function pedir(port: number): Promise<{ status: number; body: any }> {
  return new Promise((resolve, reject) => {
    const req = http.request({ port, path: '/api/uploads/photo', method: 'POST' }, (res) => {
      let raw = '';
...
describe('uploadRateLimit', () => {
  ...
  beforeAll((done) => {
    const app = express();
    app.post('/api/uploads/photo', uploadRateLimit, (_req, res) => res.json({ success: true }));
    server = app.listen(0, () => {
  ...
  it('permite las primeras 30 peticiones y bloquea la 31 con 429/RATE_LIMITED', async () => {
    for (let i = 0; i < 30; i++) {
